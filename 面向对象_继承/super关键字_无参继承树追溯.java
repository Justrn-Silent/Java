...
class 子类 extends 父类{
	 子类() {
		//super();//这里加与不加都一样，隐式调用
		//因为在在构造方法中第一行默认[前提父类构造方法是无参构造方法]会执行super()也就是会调用父类构造方法
		System.out.println("子类被调用");
	}
}
public class super关键字_无参继承树追溯 {
	public static void main(String[] args) {
		new 子类();//所以会先执行父类构造方法紧接着再子类的构造方法
	}
	
}
/*
 关于super()的运行逻辑：
  首先创建子类的实例，执行子类构造方法隐式调用super到父类的构造方法，接着父类构造方法隐式调用super到Object类的构造方法
